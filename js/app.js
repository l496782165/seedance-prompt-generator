        ...
        container.innerHTML = `
            <div class="detail-header">
                ...
                <div class="script-header-actions">
                    <button class="btn btn-secondary" onclick="App.showUploadScriptArea()" title="上传剧本">
        ...
        container.innerHTML = `
            <div class="settings-section">
                ...
                <div class="form-group">
                    <label for="doubao-api-endpoint">API 端点</label>
                    <input type="text" id="doubao-api-endpoint" class="form-input" value="${settings.doubaoApiEndpoint || 'https://ark.cn-beijing.volces.com/api/v3/responses'}" placeholder="https://ark.cn-beijing.volces.com/api/v3/responses">
                    <p class="form-hint">豆包API端点地址</p>
                </div>
                <div class="form-group">
                    <label for="doubao-api-key">API Key</label>
                    <input type="password" id="doubao-api-key" class="form-input" value="${settings.doubaoApiKey || ''}" placeholder="请输入您的豆包API Key">
                    <p class="form-hint">您的API密钥将安全存储在本地</p>
                </div>
                <div class="form-group">
                    <label for="doubao-model-id">模型 ID</label>
                    <input type="text" id="doubao-model-id" class="form-input" value="${settings.doubaoModelId || 'doubao-seed-2-0-lite-260215'}" placeholder="doubao-seed-2-0-lite-260215">
                    <p class="form-hint">豆包模型标识符</p>
                </div>
            </div>
            ...
            <div class="settings-section">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="api-endpoint" class="form-input" value="${settings.apiEndpoint || 'https://open.xiaojingai.com/v1'}" placeholder="https://open.xiaojingai.com/v1">
                    <p class="form-hint">提示词生成API端点</p>
                </div>
                <div class="form-group">
...
const App = {
    ...
    saveSettings() {
        ...
        const doubaoApiEndpoint = document.getElementById('doubao-api-endpoint')?.value.trim();
        const doubaoApiKey = document.getElementById('doubao-api-key')?.value.trim();
        const doubaoModelId = document.getElementById('doubao-model-id')?.value.trim();

        if (!doubaoApiEndpoint) {
            Utils.showNotification('请输入豆包API端点', 'error');
            return;
        }

        if (!doubaoApiKey) {
            Utils.showNotification('请输入豆包API Key', 'error');
            return;
        }
        ...
        StorageService.saveSettings({ 
            ...
            doubaoApiEndpoint,
            doubaoApiKey,
            doubaoModelId: doubaoModelId || 'doubao-seed-2-0-lite-260215'
        });
        AppState.settings = StorageService.getSettings();
     ...
     * 显示剧本上传区域
     */
    showUploadScriptArea() {
        // 如果已有上传区域，返回
        if (document.querySelector('.file-upload-wrapper')) return;

        const header = document.querySelector('.detail-header');
        const actionsDiv = document.createElement('div');
        actionsDiv.className = 'script-header-actions';
        actionsDiv.innerHTML = `
            <div class="file-upload-wrapper">
                <div class="file-upload-area" id="script-upload-area">
                    <div class="file-upload-icon">📄</div>
                    <p class="file-upload-text">
                        拖拽剧本文件到此处，或 <strong>点击上传</strong>
                    </p>
                    <p class="file-upload-hint">支持 .txt 和 .md 格式</p>
                </div>
                <input type="file" class="file-upload-input" id="script-file-input" accept=".txt,.md">
            </div>
        `;
        
        // 在设置按钮前插入
        const settingsBtn = header.querySelector('.btn-settings');
        header.insertBefore(actionsDiv, settingsBtn);
    ...
    bindUploadEvents() {
        ...
        uploadArea.addEventListener('drop', (e) => {
            ...
            if (e.dataTransfer.files && e.dataTransfer.files[0]) {
                this.handleScriptFile(e.dataTransfer.files[0]);
            }
        });
    },
    ...
    handleScriptFile(file) {
        ...
        if (!validExtensions.includes(fileExtension)) {
            Utils.showNotification('请上传 .txt 或 .md 格式的文件', 'error');
            return;
        }
        ...
        reader.onload = (e) => {
            ...
            this.showScriptPreview(content, file.name);
        };
        reader.onerror = () => {
     ...
     * 显示剧本预览
     */
    showScriptPreview(content, fileName) {
        const modal = document.getElementById('modal-script-preview');
        const contentDiv = document.getElementById('script-preview-content');
     ...
     * 分析剧本内容
     */
    async analyzeScriptContent() {
        if (!this.currentScriptContent) {
            Utils.showNotification('请先上传剧本文件', 'error');
            return;
        }

        const settings = StorageService.getSettings();
        
        if (!settings.doubaoApiEndpoint || !settings.doubaoApiKey) {
            Utils.showNotification('请先在设置中配置豆包API', 'error');
        ...
        // 关闭预览模态框
        document.getElementById('modal-script-preview').classList.remove('show');

        try {
            const analysisResult = await this.callDoubaoAPI(this.currentScriptContent);
            
            // 解析结果
            this.parseAnalysisResult(analysisResult);
            
            // 显示分析结果模态框
            document.getElementById('modal-script-analysis').classList.add('show');
            
        } catch (error) {
            console.error('分析失败:', error);
            Utils.showNotification('分析失败：' + error.message, 'error');
        } finally {
            // 隐藏加载动画
...
注意：
1. 仔细阅读剧本，识别所有出现的人物角色
2. 提取角色的外貌特征、服装特点、使用的武器等
3. 识别剧本中的主要场景
4. 识别重要的道具物品
5. 只返回JSON，不要有其他文字`;
    ...
    parseAnalysisResult(resultText) {
        try {
            ...
            // 更新计数
            document.getElementById('analysis-char-count').textContent = this.analysisData.characters.length;
            document.getElementById('analysis-scene-count').textContent = this.analysisData.scenes.length;
            document.getElementById('analysis-prop-count').textContent = this.analysisData.props.length;
            
            // 渲染各选项卡内容
            this.renderAnalysisCharacters();
            this.renderAnalysisScenes();
            this.renderAnalysisProps();
            
        } catch (error) {
            console.error('解析失败:', error);
            throw new Error('无法解析分析结果，请重试');
        }
    },
     ...
     * 渲染分析的角色列表
     */
    renderAnalysisCharacters() {
        const container = document.getElementById('analysis-characters-list');
        
        if (!this.analysisData.characters.length) {
            container.innerHTML = `
        ...
        container.innerHTML = `
            <div class="analysis-list">
                ${this.analysisData.characters.map((char, index) => `
                    <div class="analysis-item" data-index="${index}">
                        <div class="analysis-item-header">
                            <h4>角色 ${index + 1}</h4>
                            <input type="checkbox" checked data-type="character" data-index="${index}">
                        </div>
                        <div class="analysis-item-body">
                            <div class="analysis-item-row">
                                <label>名称</label>
                                <input type="text" value="${this.escapeHtml(char.name || '')}" data-field="name">
                            </div>
                            <div class="analysis-item-row">
                                <label>外貌</label>
                                <textarea data-field="appearance">${this.escapeHtml(char.appearance || '')}</textarea>
                            </div>
                            <div class="analysis-item-row">
                                <label>服装</label>
                                <input type="text" value="${this.escapeHtml(char.clothing || '')}" data-field="clothing">
                            </div>
                            <div class="analysis-item-row">
                                <label>武器</label>
                                <input type="text" value="${this.escapeHtml(char.weapon || '')}" data-field="weapon">
                            </div>
                        </div>
                    </div>
                `).join('')}
        ...
        container.querySelectorAll('input, textarea').forEach(input => {
            input.addEventListener('change', (e) => {
                ...
                if (this.analysisData.characters[index]) {
                    this.analysisData.characters[index][field] = e.target.value;
                }
            });
        });
    },
     ...
     * 渲染分析的场景列表
     */
    renderAnalysisScenes() {
        const container = document.getElementById('analysis-scenes-list');
        
        if (!this.analysisData.scenes.length) {
            container.innerHTML = `
                <div class="analysis-empty">
                    <div class="analysis-empty-icon">🎬</div>
                    <p>未检测到场景</p>
        ...
        container.innerHTML = `
            <div class="analysis-list">
                ${this.analysisData.scenes.map((scene, index) => `
                    <div class="analysis-item" data-index="${index}">
                        <div class="analysis-item-header">
                            <h4>场景 ${index + 1}</h4>
                            <input type="checkbox" checked data-type="scene" data-index="${index}">
                        </div>
                        <div class="analysis-item-body">
                            <div class="analysis-item-row">
                                <label>名称</label>
                                <input type="text" value="${this.escapeHtml(scene.name || '')}" data-field="name">
                            </div>
                            <div class="analysis-item-row">
                                <label>描述</label>
                                <textarea data-field="description">${this.escapeHtml(scene.description || '')}</textarea>
                            </div>
                        </div>
                    </div>
                `).join('')}
        ...
        container.querySelectorAll('input, textarea').forEach(input => {
            input.addEventListener('change', (e) => {
                ...
                const index = parseInt(item.dataset.index);
                const field = e.target.dataset.field;
                if (this.analysisData.scenes[index]) {
                    this.analysisData.scenes[index][field] = e.target.value;
                }
            });
        });
    },
     ...
     */
    renderAnalysisProps() {
        const container = document.getElementById('analysis-props-list');
        
        if (!this.analysisData.props.length) {
            container.innerHTML = `
                <div class="analysis-empty">
                    <div class="analysis-empty-icon">🎁</div>
                    <p>未检测到道具</p>
        ...
        container.innerHTML = `
            <div class="prop-tags">
                ${this.analysisData.props.map((prop, index) => `
                    <div class="prop-tag">
                        <input type="checkbox" checked data-type="prop" data-index="${index}">
                        <input type="text" value="${this.escapeHtml(prop.name || '')}" data-field="name">
                    </div>
                `).join('')}
            </div>
        `;
        ...
        container.querySelectorAll('input[type="text"]').forEach(input => {
            input.addEventListener('change', (e) => {
                ...
                if (checkbox && checkbox.type === 'checkbox') {
                    ...
                    if (this.analysisData.props[index]) {
                        this.analysisData.props[index].name = e.target.value;
                    }
                }
            });
        });
     ...
     * 确认添加提取的数据
     */
    confirmAddExtractedData() {
        if (!this.analysisData) {
            Utils.showNotification('没有可添加的数据', 'error');
            return;
        }

        const modal = document.getElementById('modal-script-analysis');
        
        // 获取选中的角色
        const selectedCharacters = [];
        modal.querySelectorAll('input[data-type="character"]:checked').forEach(checkbox => {
            const index = parseInt(checkbox.dataset.index);
            const charData = this.analysisData.characters[index];
        ...
        selectedCharacters.forEach(char => {
            ScriptService.addCharacter(AppState.currentScriptId, char);
        });
        ...
        if (selectedCharacters.length > 0) {
            Utils.showNotification(`已添加 ${selectedCharacters.length} 个角色到角色库`, 'success');
        } else {
            Utils.showNotification('未选择任何角色', 'info');
        }
    }
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 剧本分析模态框 Tab切换
    document.getElementById('modal-script-analysis')?.addEventListener('click', (e) => {
        const tabBtn = e.target.closest('.analysis-tabs .tab-btn');
        if (tabBtn) {
            const tabName = tabBtn.dataset.analysisTab;
            
            // 更新Tab按钮状态
            document.querySelectorAll('#modal-script-analysis .analysis-tabs .tab-btn').forEach(btn => {
                btn.classList.toggle('active', btn.dataset.analysisTab === tabName);
            });
            
            // 更新Tab内容
            document.querySelectorAll('#modal-script-analysis .analysis-tab-content').forEach(content => {
                content.classList.toggle('active', content.id === `analysis-tab-${tabName}`);
            });
        }
    });
